...
#         return x
class PatchEmbed(nn.Module):
    """ Image/Video to Patch Embedding (2D conv projection, supports both image and video input)

    - If input is 4D: (B, C, H, W) -> produce (B, 1, HW, C) (T=1)
    - If input is 5D: (B, C, T, H, W) -> treat as (B*T, C, H, W) and produce (B, T, HW, C)
    This design lets us load 2D pretrained weights [out, in, kh, kw] directly.
    """
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, norm_layer=None, flatten=True):
        ...
        # use Conv2d so pretrained 2D weights match directly: weight shape [out, in, kh, kw]
    ...
    def forward(self, x):
        ...
        if x.dim() == 4:
            ...
            if self.flatten:
                ...
                # add a fake temporal dim to keep API: return (B, T, HW, C) with T=1
                x = x.unsqueeze(1)  # (B, 1, HW, C)
            else:
                # if not flatten, keep (B, embed_dim, H', W')
                pass
            x = self.norm(x)
            return x

        elif x.dim() == 5:
            # (B, C, T, H, W) -> treat per-frame as images
            B, C, T, H, W = x.shape
            assert H == self.img_size[0] and W == self.img_size[1], \
                f"Input image size ({H}*{W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."
            # merge B and T to (B*T, C, H, W)
            xt = x.permute(0, 2, 1, 3, 4).reshape(B * T, C, H, W)
            x_proj = self.proj(xt)  # (B*T, embed_dim, H', W')
            if self.flatten:
                x_proj = x_proj.flatten(2).permute(0, 2, 1)  # (B*T, HW, C)
                # reshape to (B, T, HW, C)
                x_proj = x_proj.view(B, T, x_proj.shape[1], x_proj.shape[2])
            else:
                # keep other format if desired
                pass
            x_proj = self.norm(x_proj)
            return x_proj
        else:
            raise RuntimeError(f"Unsupported input dim {x.dim()} for PatchEmbed")
...
def internvl_clip_6b(
    ...
    ckpt = torch.load(_MODELS["internvl_c_13b_224px"], map_location='cuda')
